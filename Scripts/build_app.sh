...
SWIFTTERM_DISABLE_REFLOW_MIGRATION_PATCH="$ROOT/Patches/SwiftTerm-disable-resize-reflow-migration.patch"
...
if [[ -f "$SWIFTTERM_BUFFER" ]] && grep -q "termu reflows only when wrapped lines are full-width scrollback rows" "$SWIFTTERM_BUFFER"; then
    chmod u+w "$SWIFTTERM_BUFFER"
    patch -d "$SWIFTTERM_ROOT" -p1 < "$SWIFTTERM_DISABLE_REFLOW_MIGRATION_PATCH"
elif [[ -f "$SWIFTTERM_BUFFER" ]] && ! grep -q "termu disables resize reflow" "$SWIFTTERM_BUFFER"; then
    chmod u+w "$SWIFTTERM_BUFFER"
